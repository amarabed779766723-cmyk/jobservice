...
<div style="max-width:600px; margin:0 auto; padding:1.5rem 1rem;">
    ...
    @if($chats->isEmpty())
        <div class="card"><p>لا توجد محادثات بعد.</p></div>
    @endif
        ...
        <a href="{{ route('chat.show', $chat->id) }}" style="text-decoration:none; color:inherit;">
            <div class="card" style="display:flex; align-items:center; gap:1rem;">
                ...
                <div style="flex:1;">
                    ...
                    <p style="color:var(--text-secondary); margin:0;">{{ $chat->messages->first()->message_text ?? 'بدون رسائل' }}</p>
                </div>
...
<div id="chatSettingsModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.7); z-index:9999; justify-content:center; align-items:center;">
    <div style="background:var(--bg-card); border-radius:12px; padding:1.5rem; width:90%; max-width:400px;">
        <h3 style="margin-bottom:1rem;">⚙️ إعدادات الدردشة</h3>
        
        <div style="margin-bottom:1rem;">
            <label style="font-weight:500; display:block; margin-bottom:0.5rem;">🎨 خلفية الدردشة</label>
            <form method="POST" action="{{ route('settings.chat_wallpaper') }}" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-primary btn-sm" style="margin-top:0.5rem;">تغيير</button>
            </form>
        </div>

        <button onclick="toggleChatSettings()" class="btn btn-outline btn-sm" style="width:100%;">إغلاق</button>
    </div>
</div>
